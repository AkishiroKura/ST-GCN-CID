...
#         A = np.load('data2_tr5/adj_matrix.npy')
#         Dl = np.sum(A, 0)
#         num_node = A.shape[0]
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(self.edge_importance.shape)
        for gcn in self.st_gcn_networks:
            x, _ = gcn(x, self.A * (self.edge_importance * self.edge_importance + torch.transpose(
                self.edge_importance * self.edge_importance, 1, 2)))
        x = self.eca(x)
        # global pooling
        x = F.avg_pool2d(x, x.size()[2:])
        x = x.view(N, M, -1, 1, 1).mean(dim=1)
    ...
    def extract_feature(self, x):
        ...
        # global pooling
        # 在全局池化和全连接层之前提取特征
        pooled_x = F.avg_pool2d(x, x.size()[2:])
        feature = pooled_x.view(N, M, -1).mean(dim=1)

        # prediction
        # 为了保持一致性，也返回最终的预测结果
        output = self.fcn(feature.view(N, M, -1, 1, 1).mean(dim=1))
